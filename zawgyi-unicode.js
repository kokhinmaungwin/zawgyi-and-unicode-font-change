(function(){
  // avoid admin/editor interference
  if (/(\/b\/|\/admin|post-edit|draft)/i.test(location.href)) return;

  // check Myanmar text exists
  function hasMyanmarText() {
    try {
      return /[\u1000-\u109F]/.test(document.body.innerText);
  ...
  // 💡 INSERT YOUR CONVERTER FUNCTIONS HERE
  // =========================================
...
function leikpyar_init(){
	...
	addStyle({css:'.converter span{vertical-align:middle;font-family:Arial;}.converter{top:0px;left:0;z-index:99999;padding:5px;text-align:center;position:fixed;border:#87CEFA solid 1px;background-color:#e5ecf9;filter:alpha(opacity=80);-moz-opacity:0.8;opacity:0.8;font-family:Arial;}'});
...
function _switch_font(_opt){
	...
	var  txtdiv = document.createElement("div");txtdiv.id="leikpyar_converter";txtdiv.className='converter';txtdiv.innerHTML = '<span style="cursor:pointer;" onclick="_deactivate(\'leikpyar_converter\');">[X]&nbsp;<\/span><span>Choose Font Here &nbsp;<\/span><span><select  onchange="_switch_font(this.value);"><option value=1'+(_curFont==1?' selected':'')+'>Zawgyi-One<\/option><option value=2'+(_curFont==2?' selected':'')+'>Unicode<\/option><\/select><\/span>';
...
function getcookie(_s) {
  ...
  function walk(node, mode){
    ...
    while(c){
      ...
      if(c.nodeType===3){
        try{
          if(mode==='zawgyi'){ c.nodeValue = Z1_Uni(c.nodeValue); }
          else { c.nodeValue = Uni_Z1(c.nodeValue); }
        }catch(e){}
      } else if(c.nodeType===1){
        const tag=c.tagName;
        if(!['SCRIPT','STYLE','TEXTAREA'].includes(tag) && c.id!=='mm-font-selector'){
          walk(c, mode);
        }
      }
      c=next;
  ...
  function apply(mode){
    ...
    if(mode==='zawgyi'){
      document.documentElement.style.fontFamily="Zawgyi-One, Arial, sans-serif";
      writeCookie("mmfont","zg",365);
    } else {
      document.documentElement.style.fontFamily="Myanmar3, Padauk, Parabaik, sans-serif";
      writeCookie("mmfont","uni",365);
    }
  }

  function init(){
    if(!hasMyanmarText()) return;

    // create UI
    const div=document.createElement("div");
    div.id="mm-font-selector";
    div.style=`position:fixed;top:15px;right:15px;z-index:99999;background:#fff;
              padding:6px 10px;border-radius:8px;border:1px solid #ddd;
              font-family:Arial;font-size:14px`;
    div.innerHTML=`<select id="fontSel">
      <option value="uni">Unicode</option>
      <option value="zg">Zawgyi</option>
    </select>`;
    document.body.appendChild(div);

    const saved=readCookie("mmfont")||"uni";
    document.getElementById("fontSel").value=saved;
    apply(saved);
